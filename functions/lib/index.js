...
exports.setSuperadminClaim = exports.deleteUserAndData = void 0;
/**
 ...
 */
exports.deleteUserAndData = (0, https_1.onCall)(async (request) => {
    ...
    if (request.auth.token.superadmin !== true) {
        logger.error(`El usuario ${request.auth.uid} sin permisos de superadmin intentó eliminar a otro usuario.`);
        throw new https_1.HttpsError("permission-denied", "This function can only be called by a superadmin.");
    }
    // --- 2. Validación de los Datos de Entrada ---
    const userIdToDelete = request.data.userIdToDelete;
    if (!userIdToDelete || typeof userIdToDelete !== "string") {
        throw new https_1.HttpsError("invalid-argument", "The function must be called with a 'userIdToDelete' string argument.");
    }
    // Evitar que un superusuario se elimine a sí mismo accidentalmente.
    if (request.auth.uid === userIdToDelete) {
        throw new https_1.HttpsError("failed-precondition", "Superadmins cannot delete their own account via this function.");
    }
    logger.info(`Petición del superadmin ${request.auth.uid} para eliminar al usuario ${userIdToDelete}.`);
    try {
        ...
        throw new https_1.HttpsError("internal", "Failed to delete user and their data.", error);
    }
});
/**
...
exports.setSuperadminClaim = (0, https_1.onCall)(async (request) => {
    ...
    if (request.auth?.token.superadmin !== true) {
        logger.error(`Usuario ${request.auth?.uid} intentó asignar rol de superadmin sin permisos.`);
        throw new https_1.HttpsError("permission-denied", "This function can only be called by a superadmin.");
    }
    const targetEmail = request.data.email;
    if (typeof targetEmail !== "string" || !targetEmail) {
        throw new https_1.HttpsError("invalid-argument", "La función debe ser llamada con el email del usuario.");
    }
    try {
        // const auth = getAuth();
        const userToUpdate = await auth.getUserByEmail(targetEmail);
        // Asigna el custom claim
        ...
        throw new https_1.HttpsError("internal", "No se pudo asignar el rol de superadmin.");
    }
});
